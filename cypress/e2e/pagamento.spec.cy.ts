describe('Tela de Pagamento', () => {
  ...
  it('deve preencher e enviar o formulário do cartão de crédito', () => {
    ...
    cy.get('input[formControlName="numeroCartao"]').type('4111111111111111', {
      force: true,
    });
    cy.get('input[formControlName="nomeCartao"]').type('FULANO DE TAL', {
      force: true,
    });
    cy.get('input[formControlName="mesValidade"]').type('12', { force: true });
    cy.get('input[formControlName="anoValidade"]').type('2025', {
      force: true,
    });
